...
spk <- cbind(data, QTY_CNT)
spk <- subset(spk, select = -c(X, CATEGORY, QTY, ITEM_CNT))
spk
```
...
#회귀 모델
m1=lm(MAXTEMP~QTY_CNT,data = spk)
m2=lm(PRICE~QTY_CNT,data = spk)
m3=lm(SALEDAY~QTY_CNT,data = spk)
m4=lm(RAIN_DAY~QTY_CNT,data = spk)
m1
m2
m3
m4
...
```{r}
m5 <- lm(QTY_CNT~MAXTEMP+PRICE+SALEDAY+RAIN_DAY, data=spk)
summary(m5)
